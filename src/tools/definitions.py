...
ALL_TOOL_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    # --- DP-227: fixr self-improvement supervisor (service_binding "fixr") -----
    # dispatch_fix is the ONE always-gated write: it parks for human approval
    # before a coding agent ever runs. The other four are is_write:False so the
    # woken supervisor can coordinate (answer/kill) and report (discord) without
    # a confirmation prompt on every event — "gate every dispatch, dial later".
    {
        "type": "function",
        "is_write": True,
    ...
    # --- DP-238: voice command timers (service_binding "voice") ----------------
    # Shared with the spoken always-listening pipeline; these expose the same
    # TimerService to the LLM so timers work from typed conversation too. All
    # low-risk/reversible → is_write False (a timer announces in a channel and
    # can be cancelled).
    {
        "type": "function",
        "is_write": False,
