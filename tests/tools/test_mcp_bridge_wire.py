...
async def test_real_claude_subprocess_calls_a_bridge_tool():
    ...
    Pinned to the cheapest model by default: what is under test is the
    ``--mcp-config`` handshake, which every model performs identically, so
    paying for a larger one buys nothing. Override with ``LLM_LIVE_CC_MODEL``
    if a connect failure needs to be checked against another model.
    """
    import os
    import shutil
    ...
    async with _serve() as served:
        ...
        proc = await asyncio.create_subprocess_exec(
            "claude", "-p",
            "--model", os.environ.get("LLM_LIVE_CC_MODEL", "haiku"),
