...
def memory_agent(mock_load, mock_chat_system):
...
class TestMemoryAgentInit:
    @patch('src.agents.base.load_system_personas_from_file', return_value={})
    def test_init_stores_config(self, mock_load, mock_chat_system):
        agent = MemoryAgent(mock_chat_system, agent_config={
    ...
    @patch('src.agents.base.load_system_personas_from_file', return_value={})
    def test_init_defaults(self, mock_load, mock_chat_system):
        agent = MemoryAgent(mock_chat_system)
        assert agent._similarity_threshold == 0.3
        assert agent._min_segment_size == 3
        assert agent._batch_size == 200

    @patch('src.agents.base.load_system_personas_from_file', return_value={})
    def test_lazy_embedding_service(self, mock_load, mock_chat_system):
        agent = MemoryAgent(mock_chat_system)
...
class TestSegmentation:
    def _no_seed(self, memory_agent):
    ...
    def test_all_similar_messages_one_segment(self, memory_agent):
    ...
    def test_orthogonal_messages_multiple_segments(self, memory_agent):
    ...
    def test_min_segment_size_enforced(self, memory_agent):
    ...
    def test_too_few_messages(self, memory_agent):
        """Empty input produces no segments."""
        self._no_seed(memory_agent)
        segments = memory_agent._segment_by_similarity([], [], "chan", "p1", None)
        assert len(segments) == 0

    def test_centroid_renormalization(self, memory_agent):
...
class TestCentroidSeeding:
    def test_seed_from_previous_segment(self, memory_agent):
    ...
    def test_seed_returns_none_when_no_previous(self, memory_agent):
    ...
    def test_topic_continuation_across_batches(self, memory_agent):
...
class TestSummarization:
    @pytest.mark.asyncio
    async def test_summarize_calls_llm(self, memory_agent):
    ...
    @pytest.mark.asyncio
    async def test_summarize_handles_missing_persona(self, memory_agent):
    ...
    @pytest.mark.asyncio
    async def test_summarize_handles_llm_error(self, memory_agent):
...
class TestDeploy:
    ...
    async def test_deploy_processes_channels(self, mock_provider_cls, memory_agent):
    ...
    @pytest.mark.asyncio
    @patch('src.agents.memory_agent.GeminiEmbeddingProvider')
    async def test_deploy_no_channels(self, mock_provider_cls, memory_agent):
    ...
    @pytest.mark.asyncio
    @patch('src.agents.memory_agent.GeminiEmbeddingProvider')
    async def test_deploy_skips_below_min_segment_size(self, mock_provider_cls, memory_agent):
        ...
        # Only 2 messages, min_segment_size is 3
        memory_agent.memory_manager.get_unembedded_messages.return_value = [
            {'interaction_id': 1, 'content': 'hi', 'author_role': 'user', 'author_name': 'Alice'},
            {'interaction_id': 2, 'content': 'hello', 'author_role': 'user', 'author_name': 'Bob'},
        ]
        memory_agent.memory_manager.get_last_segment_tail_embeddings.return_value = None
    ...
    @pytest.mark.asyncio
    @patch('src.agents.memory_agent.GeminiEmbeddingProvider')
    async def test_deploy_respects_shutdown(self, mock_provider_cls, memory_agent):
    ...
    @pytest.mark.asyncio
    @patch('src.agents.memory_agent.GeminiEmbeddingProvider')
    async def test_deploy_continues_on_channel_error(self, mock_provider_cls, memory_agent):
...
class TestConfigInjection:
    @patch('src.agents.base.load_system_personas_from_file', return_value={})
    def test_batch_size_from_config(self, mock_load, mock_chat_system):
