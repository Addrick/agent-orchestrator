...
|---|---|---|
| Park a write call for human approval | `confirmations.ConfirmationManager` (chat turns, in-memory, keyed by `token`, **N pending per user+persona** since DP-297, each resolved by its own continuation turn) · `proposals` queue (agent writes, SQLite `Proposals`, `proposal_id`, approved by LLM tool, run by `ProposalExecutor`) | `by design` — ADR 2026-07-04; the in-memory one is chat-turn-bound and cannot hold a write across restarts. `tools/mcp_bridge.py:192` correctly reuses `create_proposal` rather than adding a third. **DP-297 narrowed the gap to exactly one property: durability.** Convergence was considered and rejected for now — `ProposalExecutor` dispatches a *whitelist* of `action_type`s (`executor.py:108-135`), and that whitelist IS the privilege separation, so hosting arbitrary chat tool calls would mean deleting it. Any future convergence needs its own decision record first. |
...
| Suppress a duplicate proposal | `tool_loop.write_call_identity` (tool name + canonicalized arguments, call id excluded) consulted via the `pending_lookup` closure `ChatSystem._orchestrate` injects | Only implementation. Introduced DP-297. Note the tool result's `instruction` text ("do not re-submit") is *not* a second implementation — it is a request to the model; this is the enforcement. Keyed on **pending**, not on history, so a decided action can legitimately be proposed again. The predicate is injected rather than imported because `ConfirmationManager` sits above `src.tools` in the layer order. |
