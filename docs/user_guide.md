...
The card also shows the **KV precision** the persona chose (DP-364). That is
the one thing on the card that *is* the model's proposal rather than a fact read
from the Hub, so it is the part to check against the persona's reasoning. A
value the node would refuse is flagged `⚠️ INVALID` on the card, so you don't
approve an install that is certain to fail.
