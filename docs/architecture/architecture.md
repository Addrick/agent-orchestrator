...
- `drop_orphaned_tool_head(messages)` -> `(repaired, dropped_count)` (SYS-PAYLOAD-001). Every truncation here counts **messages, not turns** — both the DB sliding window (row `LIMIT`, expanded into the full tool sequence by `format_raw_history_for_llm`) and `truncate_messages_to_budget` can cut mid-sequence and leave the history *starting* on an `assistant`-with-`tool_calls` or a `tool` message. Google rejects that shape with 400 *"function call turn must come immediately after a user turn"* (and the OAI/Anthropic wire formats reject an unpaired tool result too), so the dangling head is dropped. Called once, last thing in `prepare_request`, after both truncations.
