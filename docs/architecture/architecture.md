...
- `PendingConfirmationEvent(token, write_call, audit_info, confirmation_text, turn_tainted)` -- one gated write surfaced mid-stream as its own approve/deny affordance
- `GenerationEvent` Union; `ResponseType` enum: DEV_COMMAND, LLM_GENERATION, PENDING_CONFIRMATION. **PENDING_CONFIRMATION is vestigial as of DP-297** — nothing produces it any more (a turn that parks writes still ends with real text and persists as LLM_GENERATION). It survives only so old rows and the retry branch in `turn_persistence` keep their meaning.
...
- PENDING_ACTION_TTL = 86400 (seconds; DP-297 renamed + raised it from PENDING_CONFIRMATION_TIMEOUT = 300, since a park became a queue worked through later rather than a blocking modal. ⚠️ the store is in-memory, so the effective TTL is min(this, process uptime) — see DP-319)
