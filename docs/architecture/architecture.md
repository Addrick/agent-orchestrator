...
- `drop_orphaned_tool_head(messages, injected=None)` -> `(repaired, dropped_count)` (DP-298). Every truncation here counts **messages, not turns** — both the DB sliding window (row `LIMIT`, expanded into the full tool sequence by `format_raw_history_for_llm`) and `truncate_messages_to_budget` can cut mid-sequence and leave the history *starting* on an `assistant`-with-`tool_calls` or a `tool` message. Google rejects that shape with 400 *"function call turn must come immediately after a user turn"* (leading call) or *"function response turn must come immediately after a function call turn"* (leading bare result), and the OAI/Anthropic wire formats reject an unpaired tool result too, so the dangling head is dropped. The scan walks **through** a leading run of non-conversational preamble before looking for the orphan: `role=='system'` entries (which `truncate_messages_to_budget` deliberately preserves while dropping the conversation around them, and which the `client_messages` path only strips one of) plus anything in `injected`, matched by **identity**. `prepare_request` passes the LTM recall block as `injected` — it is inserted at index 0 as a `user` message but is not a real conversational turn, and without the skip it shielded the orphan behind it. Identity matching means a caller can pass a block that the pruner may or may not have dropped and get the right answer either way. Called once, last thing in `prepare_request`, **after** both truncations — the token-prune itself can open a new orphan, which is why this is a preamble-skip rather than simply running the repair earlier.
