...
- Provider-agnostic LLM abstraction: OpenAI, Anthropic, Google (Gemini/Gemma), local kobold-native, Antigravity (`agy`)
...
Pure helpers for enforcing per-persona `max_context_tokens`. Two callsites today: `request_builder.prepare_request` and `kobold_adapter` `/chat/completions`. Future home for dynamic LTM-depth modulation, retrieval-score-weighted budget allocation, and a real tokenizer swap.
