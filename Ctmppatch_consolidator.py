...
def _build_response_model(max_creates: int | None = None) -> type[_ConsolidationBatchResponse]:
    ...
    # kobold.cpp grammar sampler rejects high repetition counts ("number of
    # repetitions exceeds sane defaults"). Cap schema-level maxItems to a
    # grammar-safe ceiling; defensive truncation downstream still enforces the
    # real observation slot count.
    GRAMMAR_SAFE_CEIL = 64
    clamped = min(max(max_creates, 0), GRAMMAR_SAFE_CEIL)
